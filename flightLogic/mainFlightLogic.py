# disable lint for micro python files
...
# main function doesn't seem to work in MicroPython; actions in main function moved to bottom of file.
...
async def executeFlightLogic():  # Open the file save object, start TXISR, camera obj, and start Boot Mode data collection
	...
	tasks=[]
	tasks.append(uasyncio.create_task(heartBeatObj.heartBeatRun()))  # starting heart beat
	...
	# TODO: Remove 3 variables used for testing
	saveObject = None
	transmitObject = None
	packet = None
	...
	# TODO: Remove 2 variables used for testing
	antennaDeployed = True
	lastMode = 4
	...
	# pre boom deploy
		...
		if ((antennaDeployed == True) and (lastMode != 3) and (lastMode != 4)):
			...
			print('Running post-Boom Deploy')
			# recordData(bootCount, antennaDeployed, lastMode)  # Save into files
			await uasyncio.gather(postBoomDeploy.run())
...
uasyncio.run(executeFlightLogic())